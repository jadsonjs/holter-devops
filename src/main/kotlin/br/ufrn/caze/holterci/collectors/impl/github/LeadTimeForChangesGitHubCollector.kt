...
class LeadTimeForChangesGitHubCollector : Collector(UUID.fromString("250c6d7c-910e-41bc-b3c6-6f5955a41051"),  Metric.LEAD_TIME_FOR_CHANGES, "Lead time for changes at Github", MetricRepository.GITHUB) {
    ...
    override fun calcMetricValue(period: Period, globalPeriod: Period, project: Project): Pair<BigDecimal, String>  {
        ...
        executorPulls.setGithubToken(projectConfiguration.mainRepositoryToken)
        ...
        for (commit in commitsOfPeriod) { // for all commits in the period
            ...
            for (pullRequest in pullList) {
                ...
                if(pullRequest.base.ref.equals(projectConfiguration.produtionBranch.trim(), true)) { // if the pull request was to the production
